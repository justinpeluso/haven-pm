...
export function offlineComputerHelperPlan(
  query: string,
  os: HelperOs,
  liveError?: string
): ComputerHelperPlan {
  ...
  const summary = `Likely a ${t.label.toLowerCase()} issue on ${osLabel}. Follow the action plan below — start with the safest checks first. ${
    liveError
      ? "Live AI was unavailable, so this is a curated offline playbook."
      : "This is a curated offline playbook."
  }`;
  const base: ComputerHelperPlan = {
    ...
    topic: t.label,
    note: liveError
      ? `Offline fallback — ${liveError}`
      : "Offline helper — heuristic playbook (set OPENAI_API_KEY for live AI).",
    liveError,
  };
