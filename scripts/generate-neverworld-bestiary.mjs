...
function makeProps(seed, tier, slot) {
  ...
  for (let i = 0; i < count; i++) {
    let key = preferred.concat(ALL_PROP_KEYS)[(seed + i * 7) % (preferred.length + ALL_PROP_KEYS.length)];
    let guard = 0;
    while (used.has(key) && guard++ < 20) {
      key = ALL_PROP_KEYS[(seed + i * 11 + guard) % ALL_PROP_KEYS.length];
    }
    used.add(key);
    ...
    props.push({ key, value, label: propLabel(key, value) });
  }
  return props;
}
...
function withAffixes(item) {
  ...
  const tier = item.rarity === "rare" ? "rare" : item.tier;
  return {
    ...item,
    properties: signature ?? makeProps(hashId(item.id), tier, item.slot),
  };
}
...
function buildBattleLoot(bosses, spellbooks) {
  ...
  return {
    ...
    items: items.map(withAffixes),
    bossUniques: bossUniques.map(withAffixes),
  };
}
